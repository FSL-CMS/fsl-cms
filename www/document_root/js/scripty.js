...
{
	$(".hodnoceni").rating({showCancel: false}).closest('form').find('input[type=submit]').hide();
	$(".hodnoceni").bind("change", function()
	{
		$.post(
			...
			function(data)
			{
				...
				$(".hodnoceni").rating({showCancel: false, disabled: true});
			}
		);
	});
	...
	$('.handle *').show();
	$('.sortable').sortable({cursor: 'move', handle: $('.handle'), helper: 'clone'});
	$('.sortable').sortable( "option", "helper", 'clone' );
	$('.sortable').bind('sortupdate', function(event, ui)
	{
		$(this).children('tr').each( function(i) {$(this).find('input.poradi').val(i+1);} );
	...
	$("a.zvetsenina").fancybox({'type' : 'image', 'titlePosition' : 'inside'});
	$("a.vyber-z-galerie").live('click', function(e)
	{
		$.fancybox({'titleShow' : false, 'href' : $(this).attr('href'), 'autoScale' : true, 'scrolling' : 'auto'});
		return false;
	});
